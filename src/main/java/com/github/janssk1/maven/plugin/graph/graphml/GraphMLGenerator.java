  ...
  public void serialize(Graph graph, Writer writer, RenderOptions options) throws IOException
  {
    ...
    try
    {
      ...
      for (Vertex vertex : graph.getVertices())
      {
        ...
        gen.beginLeafNode("" + vertex.getArtifactIdentifier().toString(), encode(vertexInfo.label),
                          "#" + Integer.toHexString(vertexInfo.color.getRGB() & 0x00ffffff));
        gen.endLeafNode();
      }
      for (Vertex vertex : graph.getVertices())
      {
        ...
        for (Edge edge : edges)
        {
          ...
          String edgeLabel = "";
          if (options.geShowEdgeLabel())
          {
            edgeLabel = edge.originalDependency.getScope();
